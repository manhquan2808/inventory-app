...
<body>
    ...
    <script>
        ...
        function loadMessages(employee_id) {
            $.get(`/admin/messages/${employee_id}`, function(data) {
                $('.messages').html('');
...
@section('content')
    ...
    <body>
        <div class="container-fluid chat-container">
            <div class="employees-list">
                ...
                <div class="search-container">
                    ...
                    <a class="btn btn-secondary btn-sm" href="{{ route('admin.dashboard') }}" role="button">Trở về</a>
        ...
        @push('scripts-material-management')
            <script>
                ...
                function loadMessages(employee_id) {
                    $.get(`/material-management/messages/${employee_id}`, function(data) {
                        $('.messages').html('');
                        data.forEach(message => {
                            const messageClass = message.sender_id == '{{ Session::get('employee_id') }}' ? 'sent' :
                                'received';
                ...
                $("#messageForm").submit(function(event) {
                    ...
                    if (selectedEmployeeId) {
                        $.ajax({
                            url: "/material-management/broadcast",
                            method: 'POST',
                            headers: {
                                'X-Socket-Id': pusher.connection.socket_id
                            },
                        ...
                        alert("Hãy chọn người chat cùng.");
                    }
                });
            </script>
        @endpush
